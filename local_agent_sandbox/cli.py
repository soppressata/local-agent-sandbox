...
"""
CLI entrypoints for local-agent-sandbox and sandboxctl.
"""
...
from local_agent_sandbox.query import parse_query, filter_receipts, QuerySyntaxError
from local_agent_sandbox.run import RunEngine
from local_agent_sandbox.isolation import GLOBAL_COUNTERS, KernelPolicy, SecurityViolation
...
def swarm_monitor(topology_file: str, interval: float, once: bool, frames: int, no_color: bool):
    ...
    """
    topology = parse_tdl_file(topology_file)
    swarm_mgr = SwarmLifecycleManager(topology=topology)
    swarm_mgr.spin_up_swarm()
    try:
        monitor(
            swarm_mgr,
            interval=interval,
...
cli.add_command(swarm)
cli.add_command(sandboxctl)
cli.add_command(sandboxctl_status, name="status")


def main(args: Optional[List[str]] = None) -> int:
    if args is not None:
        try:
            if len(args) > 0 and args[0] in ("status", "run", "logs", "query", "sbom", "migrate-config"):
                sandboxctl.main(args=args, standalone_mode=False)
            else:
